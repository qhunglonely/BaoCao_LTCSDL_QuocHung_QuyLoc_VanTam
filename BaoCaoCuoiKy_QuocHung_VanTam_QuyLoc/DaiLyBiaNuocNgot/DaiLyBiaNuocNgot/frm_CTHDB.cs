...
namespace DaiLyBiaNuocNgot
{
    public partial class frm_CTHDB : Form
    {
        ...
        public void hienthi()
        {
            string sql = "SELECT sohdb, masp, soluong, giamgia, thanhtien FROM tb_CTHDB";
        ...
        private void btmoi_Click(object sender, EventArgs e)
        {
            ...
            un_locktext();
            setnull();
            string sql = "SELECT sohdb, masp, soluong, giamgia, thanhtien FROM tb_CTHDB where sohdb='" + sohdb + "'";
            msds.DataSource = cn.taobang(sql);
            SqlConnection con = cn.getcon();
            con.Open();
            try
        ...
        private void btluu_Click_1(object sender, EventArgs e)
        {
            if (cbhang.Text != "")
            {
                if (float.Parse(txtdg.Text) > 0 && float.Parse(txtsl.Text) > 0)
                {
                    if (themmoi == true)
                    {
                        try
        ...
        private void frm_CTHDB_Load_1(object sender, EventArgs e)
        {
            cbhd.Text = sohdb;
